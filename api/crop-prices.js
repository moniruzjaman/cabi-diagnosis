...
const BASELINE_PRICES = {
  ধান: {
    ...
    off: 42,
    average: 36,
    seasonMultipliers: { বোরো: 0.85, আমন: 0.9, আউশ: 0.88 },
    priceVolatility: "low",
    minSupportPrice: 30,
  },
  পাট: {
    ...
    off: 65,
    average: 52,
    seasonMultipliers: { "খরিপ-১": 0.8, "খরিপ-২": 0.85 },
  ...
  ভুট্টা: {
    ...
    off: 35,
    average: 28,
    seasonMultipliers: { রবি: 0.8, "খরিপ-১": 1.1 },
    priceVolatility: "medium",
    minSupportPrice: null,
  },
  পেঁয়াজ: {
    ...
    off: 80,
    average: 45,
    seasonMultipliers: { রবি: 0.5, "খরিপ-১": 1.6 },
    priceVolatility: "very_high",
    minSupportPrice: null,
  },
  রসুন: {
  ...
  মরিচ: {
    ...
    off: 200,
    average: 120,
    seasonMultipliers: { রবি: 0.5, "খরিপ-১": 1.5 },
    priceVolatility: "very_high",
    minSupportPrice: null,
  },
  "মসুর ডাল": {
...
function simulatePrice(crop, month, districtId) {
  ...
  const variation = (hash / 1000 - 0.5) * 0.2;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (mode === "profitability") {
      const results = Object.entries(PRODUCTION_DATA)
        .map(([crop, data]) => {
          const priceData = simulatePrice(crop, month, district);
    ...
    const allPrices = Object.keys(BASELINE_PRICES)
      .map((c) => simulatePrice(c, month, district))
      .filter(Boolean);
