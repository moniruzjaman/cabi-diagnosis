...
export default function VitSuggestions({ imageDataUrl, onApply, theme, autoRun = true, onResult }) {
  ...
  const run = useCallback(async (src) => {
    ...
    try {
      ...
      if (!isModelReady()) preloadModel().catch(() => { /* surfaced below */ });
  ...
  useEffect(() => {
    ...
    if (autoRun && imageDataUrl) {
      ...
      queueMicrotask(() => { if (!cancelledRef.current) run(imageDataUrl); });
    }
    ...
    return (
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            width: 14, height: 14, border: '2px solid #0f766e', borderTopColor: 'transparent',
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        <button
          ...
          style={{
            ...
            fontSize: 14, color: '#0f766e', padding: 2, lineHeight: 1,
          }}
        >
          🔄
        </button>
